...
export const SearchPage = ({ query, results, initialValues }: Props) => {
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      setSearchText(val);
      if (submitted) {
        const errs = validate({ searchText: val });
        setError(errs.searchText as string | undefined);
      }
    },
    [submitted],
  );
  ...
  const handleSubmit = useCallback(
    (event: FormEvent<HTMLFormElement>) => {
      ...
      setSubmitted(true);
      const errs = validate({ searchText });
      if (errs.searchText) {
        setError(errs.searchText as string | undefined);
        return;
      }
  ...
  const hasError = submitted && Boolean(error);
        ...
        <form onSubmit={handleSubmit}>
          <div className="flex gap-2">
            <div className="flex flex-1 flex-col">
              ...
              />
              {hasError && <span className="text-cax-danger mt-1 text-xs">{error}</span>}
            </div>
